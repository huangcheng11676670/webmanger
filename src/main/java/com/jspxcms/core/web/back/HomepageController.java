    ...
    private static final Logger logger = LoggerFactory.getLogger(HomepageController.class);
       ...
       @RequestMapping("welcome.do")
        public String welcome(HttpServletRequest request, org.springframework.ui.Model modelMap) {
            ...
            //客户总数
            Long customerNum = customerService.countCustomer();
            modelMap.addAttribute("customerNum", customerNum);
            //员工数量
            Long userNum =  userService.countByStatus();
            modelMap.addAttribute("userNum", userNum);
            //今日新增舆情
            Long sentimentNum = sentimentService.countByToday();
            modelMap.addAttribute("sentimentNum", sentimentNum);
            //舆情总数
            Long sentimentTotalNum = sentimentService.countTotal();
            modelMap.addAttribute("sentimentTotalNum", sentimentTotalNum);
            //合同到期数量
            Long contractEndNum = contractService.endContractNum();
            modelMap.addAttribute("contractEndNum", contractEndNum);
            
            return "core/homepage/welcome";
        }
       
    // @RequiresPermissions("core:homepage:welcome")
    public String welcomeback(HttpServletRequest request, org.springframework.ui.Model modelMap) {
        Site site = Context.getCurrentSite();
        User user = Context.getCurrentUser();
        modelMap.addAttribute("site", site);
        modelMap.addAttribute("user", user);
